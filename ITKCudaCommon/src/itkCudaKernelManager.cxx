                                     ...
                                     unsigned int sharedMemBytes)
{
  ...
  /*itkWarningMacro(<< "Kernel " << m_KernelContainer[kernelIdx] << " has " << params.size() << " params");
  for (int i = 0; i < params.size() ;i++)
    {
    itkWarningMacro(<< " param " << i << " : " << (void*)*(unsigned int*)params[i]);
    }

  itkWarningMacro(<< " globalWorkSize: " << globalWorkSize[0] << ", " << globalWorkSize[1] << ", " << globalWorkSize[2]);
  itkWarningMacro(<< " localWorkSize: " << localWorkSize[0] << ", " << localWorkSize[1] << ", " << localWorkSize[2]);

  itkWarningMacro(<< "blocks " << GETBLOCKSIZE(globalWorkSize[0], localWorkSize[0]) << ", " <<
